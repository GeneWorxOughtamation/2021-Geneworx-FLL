...
def fl(distance, speed):

    # fl =  follow the line
    # cd = current robot distance.
    # distance [] = array for robots distance and for helping the speed array.
    # kp = correction value
    # s = array increase
    cd = math.trunc(mm2I(robot.distance()))
    d =  distance [cd]
    Arraysize = len( distance )
    arraySize = len( speed )
    print(  "elements in array = " , len( speed ), "d =" , d  )
    while mm2I(robot.distance()) <= I2mm(Arraysize):
        print(  " current distance = ", (cd))
        print(rlight.reflection())
        s = speed [cd]
        correction = ((45 - rlight.reflection())*1)
        right_motor.run(s+correction)
        left_motor.run(s-correction)
    if arraySize -1 and Arraysize -1 <= cd:
        robot.stop()
        left_motor.brake()
        right_motor.brake()
